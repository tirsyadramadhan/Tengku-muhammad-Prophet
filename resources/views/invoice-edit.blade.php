    ...
    <form id="invoiceForm" action="{{ route('invoice.store') }}" method="POST">
      ...
      <div class="row">
        ...
        <div class="col-md-6 mb-3">
          ...
          <input type="hidden" name="delivery_id" value="{{ $currentDelivery->delivery_id }}">

          <!-- Disabled select for display only -->
          <select id="delivery_id" class="form-select select2" disabled readonly>
            <option value="{{ $currentDelivery->delivery_id }}"
              {{ $currentDelivery->delivery_id == $currentDelivery->delivery_id ? 'selected' : '' }}>
              Delivery Number: {{ $currentDelivery->delivery_no }}
            </option>
          </select>
        ...
        <div class="col-md-6 mb-3">
          ...
          <input type="datetime-local"
            ...
            id="tgl_invoice"
            class="form-control"
            value="{{ \Carbon\Carbon::parse($item->tgl_invoice)->format('Y-m-d\TH:i') }}"
            required>
