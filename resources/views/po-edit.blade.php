  ...
  <div class="col-xl-8 col-lg-10 mx-auto">
    <div class="card mb-4">
      <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="mb-0">Ubah Purchase Order</h5>
      </div>
      <div class="card-body">
        <form id="incomingPoForm" action="{{ route('po.update', $po->po_id) }}" method="POST">
          @csrf
          @method('PUT')
          ...
          <div class="row g-4">
            ...
            <div class="col-md-6">
              ...
              <input type="datetime-local" name="tgl_po" id="tgl_po"
                ...
                value="{{ date('Y-m-d\TH:i', strtotime($po->tgl_po)) }}" required>
              <div class="invalid-feedback">Tanggal wajib diisi.</div>
            </div>

            <div class="col-md-4">
              <label class="form-label fw-bold">Jumlah <span class="text-danger">*</span></label>
              <input value="{{ $po->qty }}" maxlength="3" type="text" name="qty" id="qty" class="form-control live-validate numeric-only" min="1" placeholder="Contoh: 10" readonly>
            </div>
            ...
            <div class="col-md-4">
              ...
              <input value="{{ $marginPercentage }}" maxlength="2" type="text" name="margin_percentage" id="margin_percentage" class="form-control numeric-only" placeholder="0" step="0.1">
            </div>
...
<script>
  $(document).ready(function() {
    ...
    numericInputs.forEach(input => {
      ...
      // Run this on page load to format existing values
      $('.currency-input').each(function() {
        // Get the clean integer string from Blade (e.g., "100000000")
        let rawValue = $(this).val();

        // Sync with the hidden input ID (removes '_display' from the ID)
        let targetId = $(this).attr('id').replace('_display', '');

        if (rawValue && rawValue !== '0') {
          // 1. Format the visual display using your existing function
          // We ensure it's a string because formatRupiah uses .replace()
          $(this).val(formatRupiah(rawValue.toString()));

          // 2. Set the hidden input value so it's ready for form submission
          $('#' + targetId).val(rawValue);
        } else {
          // If value is 0 or empty, clear display and set hidden to 0
          $(this).val('');
          $('#' + targetId).val(0);
        }
    ...
    $('#incomingPoForm').on('submit', function(e) {
      ...
      $.ajax({
        url: $('#incomingPoForm').attr('action'),
        method: 'POST',
        data: $('#incomingPoForm').serialize(),
