        ...
        <div class="col-md-3">
            <div class="card stat-card shadow-sm border-bottom border-info border-3">
                <div class="card-body d-flex align-items-center">
                    ...
                    <div>
                        <small class="text-muted d-block">Total Barang Dikirim</small>
                        <h5 class="mb-0 fw-bold">{{ number_format($stats['inventory']) }} Unit</h5>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', '.btn-delete', function(e) {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: `Delete Delivery: ${deliveryNo}? This action cannot be undone.`,
                icon: 'warning',
                ...
                preConfirm: () => {
                    return $.ajax({
                        ...
                        .fail(xhr => {
                            ...
                            // This closes the confirmation modal and opens a new Error modal
                            Swal.fire({
                                title: 'Action Denied!',
                                text: errorMsg,
                                icon: 'error',
                ...
                // We only trigger the success alert if the AJAX actually succeeded
        ...
        // Timer logic (unchanged, only message translated)
        function initTimers() {
            ...
            window.timerInterval = setInterval(function() {
                $('.timer-wrapper').each(function() {
                    ...
                    var d = days > 0 ? days + "h " : "";
                    var h = (hours < 10 ? "0" + hours : hours) + "j ";
                    var m = (minutes < 10 ? "0" + minutes : minutes) + "m ";
                    var s = (seconds < 10 ? "0" + seconds : seconds) + "d";
        ...
        function autoDeliver(id) {
            $.ajax({
                ...
                error: function() {
                    console.error("Gagal mengubah status pengiriman otomatis untuk " + id);
                }
            });
        }
    });
