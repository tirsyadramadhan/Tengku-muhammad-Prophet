    ...
    <div class="col-xl-8 col-lg-10 mx-auto">
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">Create Purchase Order (Finalize Incoming)</h5>
                <small class="text-muted">Convert Incoming PO to Open PO</small>
            </div>
            <div class="card-body">
                <form id="createPoForm" action="{{ route('po.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        <label class="form-label fw-bold text-primary">SELECT INCOMING PO (Status 0)</label>
                        <select name="incoming_po_id" id="incoming_po_id" class="form-select select2" required>
                            ...
                            <option value="{{ $incoming->po_id }}" data-no-po="{{ $incoming->no_po }}">
                                {{ $incoming->no_po }} – {{ $incoming->nama_barang }} (Qty: {{ $incoming->qty }})
                            </option>
                            @endforeach
                        </select>
                        <div class="invalid-feedback">Please select an incoming PO.</div>
                    </div>

                    <!-- Editable PO Number -->
                    <div class="mb-4">
                        <label class="form-label fw-bold">PO Number</label>
                        <input type="text" name="no_po" id="no_po" class="form-control live-validate numeric-only" maxlength="10" placeholder="Enter PO number" required>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" id="customer_name_display" class="form-control bg-light" readonly placeholder="Will be filled automatically">
                            <input type="hidden" name="customer_id" id="customer_id">
                            <div class="invalid-feedback">Customer info is missing.</div>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="invalid-feedback">Margin is required.</div>
                        </div>
                    </div>
...
<script>
    $(document).ready(function() {
        ...
        $('.select2').select2({
            theme: 'bootstrap-5',
            width: '100%',
            placeholder: '-- Select Incoming PO --',
            allowClear: true
        });
        ...
        function validateField(element) {
            ...
            if (fieldId === 'no_po') {
                ...
                if (raw.length !== 10) {
                    setValidity(false, 'PO Number must be exactly 10 digits.');
                } else if (parseInt(raw, 10) === 0) {
                    setValidity(false, 'PO Number cannot be zero.');
                } else {
                    setValidity(true, '');
                }
            }
            ...
            // Price per Unit (currency)
                ...
                if (raw === '' || parseInt(raw, 10) === 0) {
                    setValidity(false, 'Price must be greater than zero.');
                } else {
                    setValidity(true, '');
                }
            }
            // Total Margin (currency)
                ...
                let raw = unformatIDR(value);
                if (raw === '' || parseInt(raw, 10) === 0) {
                    setValidity(false, 'Margin must be greater than zero.');
                } else {
                    setValidity(true, '');
                }
            }
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        if (data.customer) {
                            ...
                            $('#customer_name_display').val('Unknown Customer');
                            $('#customer_id').val('');
                        }
                ...
                error: function() {
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: 'Failed to fetch Incoming PO details.'
                    });
                    $('#createPoForm :input').prop('disabled', false);
                }
            });
