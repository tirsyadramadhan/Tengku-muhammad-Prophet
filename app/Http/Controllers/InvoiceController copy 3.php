...
class InvoiceController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->has('pay_now') && $request->pay_now == 1) {
            $delivery = $item->delivery; // ← was undefined $delivery
        ...
        if ($request->has('pay_now') && $request->pay_now == 0) {
            $item->payment->delete();
        }
    ...
    public function destroy($id)
    {
        ...
        $this->logDelete($item, $item, 'Invoice dengan nomor invoice ' . $item->nomor_invoice . ' yang terhubung ke delivery ' . $item->delivery->delivery_no . ' dengan PO ' . $item->delivery->po->no_po . ' (' . $item->delivery->po->nama_barang . ') ' . ' Di hapus');
