...
class DashboardController extends Controller
{
    public function showDashboard()
    {
        ...
        $stats = [
            'investasi_dikembalikan' => Margin::sum('investasi_dikembalikan'),
            'sisa_margin' => Po::where('status', '!=', 0)->sum('margin') - Margin::sum('margin_diterima'),
            'dana_tersedia' => Investasi::sum('modal_setor_awal') - Investasi::sum('modal_po_baru') + Po::where('status', '!=', 0)->sum('margin') - Investasi::sum('penarikan') - Po::where('status', '!=', 7)
                ->where('status', '!=', 0)
                ->sum('margin'),
            'total_dana_ditransfer' => Investasi::sum('dana_ditransfer'),
            // Margin Tersedia = sisa margin - margin ditahan
            'margin_tersedia' => Margin::sum('margin_tersedia') - Po::where('status', '!=', 7)
                ->where('status', '!=', 0)
                ->sum('margin'),
            ...
            'margin_diterima'     => Margin::sum('margin_diterima'),
